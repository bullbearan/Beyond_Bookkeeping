...
const SetUpGoals = async (req, res) => {
	...
	if (getMonthFromString(req.body.period) - new Date().getMonth() + 1 < 0)
		...
		thePeriod =
			...
			(new Date().getMonth() + 1);
	const goalsInput = {
		users_id: goalUsername[0][0].id,
		goal_name: req.body.title.toLowerCase(),
		goal_price: req.body.amount,
		goal_monthly_savings: (
			(parseInt(req.body.amount, 10) - parseInt(req.body.current, 10)) /
			thePeriod
		).toFixed(2),
		curr_amount: req.body.current,
		goal_period: getMonthFromString(req.body.period),
	};
	await conn.query("INSERT INTO goals SET ?", [goalsInput]);
	const posts = await conn.query(
		"SELECT goal_name, goal_price, curr_amount FROM goals WHERE users_id=?",
		[goalUsername[0][0].id]
	);
	module.exports.sthePosts = posts[0];
	await conn.end();
